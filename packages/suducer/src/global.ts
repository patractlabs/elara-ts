...
export namespace G {
    ...
    export const getSuducer = (
        chain: string,
        typ: SuducerT,
        sudId: IDT
    ): Option<Suducer> => {
        const key = `${chain.toLowerCase()}-${typ}`;
        if (!Suducers[key] || !Suducers[key][sudId]) {
            return None;
        }
