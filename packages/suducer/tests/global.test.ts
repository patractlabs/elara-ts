import { ChainConfig, Option, RpcMapT, RpcStrategy } from "elara-lib";
...
describe("chain test suit", () => {
    ...
    it("none", () => {
        ...
        expect(Option.isNone(re)).toBeTruthy;
        re = G.getAllChains();
        expect(Option.isNone(re)).toBeTruthy;
    });
    ...
    it("add", () => {
        G.addChain(chain);
        let re: any = G.getChain("Polkadot");
        expect(Option.isSome(re)).toBeTruthy;
        const c = re.value;
    ...
    it("update", () => {
        let re: any = G.getChain("Polkadot");
        expect(Option.isSome(re)).toEqual(true);
        const c = re.value;
...
describe("suducer test suit", () => {
    ...
    it("update", () => {
        suducer.topic = {
            id: "1",
            topic: "chain_subscribeNewHeads",
            params: [],
        };
        G.updateSuducer(suducer);
        let re: any = G.getSuducer(chain, type, id);
        expect(re.value.topic).toEqual({
            id: "1",
            topic: "chain_subscribeNewHeads",
            params: [],
        });
