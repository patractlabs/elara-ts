...
export const post = async (
    ...
    const req = Http.request(
        ...
        (res) => {
            ...
            log.info(
                `new rpc response: chain[${chain}] pid[${cp.pid}] body[${body}] time[${time}]`
            );
        }
    );
    req.write(body);
...
async function pathOk(url: string, host: string): PResultT<any> {
...
function dataCheck(data: string): ResultT<WsData> {
    ...
    if (!dat.id || !dat.jsonrpc || !dat.method || !dat.params) {
        return Err(
            'invalid request must be JSON {"id": string, "jsonrpc": "2.0", "method": "your method", "params": []}'
        );
    }
        ...
        req.on("end", async () => {
            ...
            try {
                ...
                return Response.Fail(
                    res,
                    'Invalid request, must be JSON {"id": number, "jsonrpc": "2.0", "method": "your method", "params": []}',
                    400
                );
            }
            // dispatch request
...
wss.on("connection", async (ws, req: any) => {
    ...
    log.info(
        `New socket connection chain ${req.chain} pid[${req.pid}], current total connections `,
        wss.clients.size
    );
    const id = puber.id;
    ...
    ws.on("message", async (data) => {
        ...
        try {
            ...
            return puber.ws.send(
                'Invalid request, must be {"id": number, "jsonrpc": "2.0", "method": "your method", "params": []}'
            );
        }
    ...
    ws.on("close", async (code, reason) => {
        log.error(
            `puber[${id}] close: ${reason}, code ${code}, current total connections `,
            wss.clients.size
        );
        if (
            reason === Puber.CloseReason.OutOfLimit ||
            reason === Puber.CloseReason.SuberUnavail
        ) {
            return; // out of limit
        }
