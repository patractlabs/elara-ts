...
namespace Kfk {
    ...
    export const onConnect = (procon: ProducerT | ConsumerT): void => {
        ...
        const topgr =
            client === "Producer"
                ? (procon as ProducerT).topic
                : (procon as ConsumerT).groupId;

        procon.c
            .connect()
            .then((re) => {
                log.info(
                    `${client} client connect [${topgr}] successfully : `,
                    re
                );
            })
            .catch((err) => {
