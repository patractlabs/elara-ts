...
///
import { randomId, getAppLogger, Option } from "elara-lib";
...
const excuteStrategyList = (
    rpcs: string[],
    chain: string,
    type: SuducerT,
    stratgy?: CacheStrategyT
) => {
...
namespace Service {
    export namespace Cacheable {
        const runSyncJob = (
            chain: string,
            second: number,
            strategy: CacheStrategyT
        ): void => {
            const rpcs = G.getCacheByType(strategy);
            if (rpcs.length < 1) {
                log.warn(
                    `no item to excute in chain ${chain} cache strategy: ${strategy}`
                );
                return;
            }
        ...
        export const run = (chains: string[]) => {
            for (let chain of chains) {
                ...
                evt.once("open", () => {
                    log.error(
                        `chain ${chain} subscribe pool event done type cache`
                    );
