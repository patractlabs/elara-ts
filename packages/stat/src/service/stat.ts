...
class Stat {
    ...
    static async _timeout(pid: any, delay) {
        ...
        let average: number | string = safeParseInt(
            await statRd.get(KEY.DELAY(pid, date))
        );
        if (average) {
            //算平均

            let requests = safeParseInt(
                await statRd.get(KEY.REQUEST(pid, date))
            );
            average = ((requests * average + delay) / (requests + 1)).toFixed(
                2
            );
    ...
    static async day(pid, date) {
        ...
        let request_updatetime = await statRd.get(
...
export function dashBoardKey(date: string, pid: IDT): string {
    const S = "Stat_";
