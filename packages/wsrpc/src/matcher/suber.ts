...
const parseReq = (dat: WsData): ResultT => {
    ...
    if (isSecondResp(dat.params)) {
        ...
        if (isErr(re)) {
            log.error(
                `parse request cache error: ${re.value}, puber has been closed.`
            );
            return Ok(true);
        }
        reqId = re.value;
        ...
        log.info(
            `unsubscribe result when puber closed,fetch subscribe request ${reqId}`
        );
    }
    ...
    if (isErr(re)) {
        //
        log.error(
            `get request cache error: ${re.value}, puber has been closed `
        );
        return Ok(true);
        process.exit(1);
    }
...
const handleUnsubscribe = (req: ReqT, dres: boolean): void => {
    ...
    if (dres === false) {
        log.error(
            `Puber[${puber.id}] unsubscribe fail: chain[${req.chain}] pid[${req.pid}] topic[${req.method}] params[${req.params}] id[${req.subsId}]`
        );
    } else {
        ...
        if (isErr(re)) {
            log.error(
                `[SBH] unsubscribe topic[${req.method}] id[${subsId}] error: ${re.value}`
            );
            process.exit(1);
        }
        ...
        log.info(
            `current topic size ${puber.topics?.size}, has event: ${
                puber.event !== undefined
            } of puber[${puber.id}]`
        );
        if (puber.topics?.size === 0 && puber.event) {
            puber.event?.emit("done");
            log.info(
                `all topic unsubescribe of puber[${puber.id}], emit puber clear done.`
            );
        }
        log.info(
            `Puber[${puber.id}] unsubscribe success: chain[${req.chain}] pid[${req.pid}] topic[${req.method}] params[${req.params}] id[${req.subsId}]`
        );
    }
};
...
const dataParse = (data: WebSocket.Data, subType: SuberTyp): ResultT => {
    ...
    if (dat.error || req.type !== ReqTyp.Sub) {
        ...
        log.info(
            "delete request cache non-subscribe: ",
            req.id,
            JSON.stringify(req)
        );
    }
    ...
    if (dat.error) {
        ...
        log.info(
            `first response of subscribe method[${req.method}] params[${req.params}] puber[${req.pubId}]: `,
            dat
        );
        ...
        if (isErr(re)) {
            return Err(
                `Set subscribe context of puber[${req.pubId}] topic[${req.method}] error: ${re.value}`
            );
        }
        log.info(
            `Puber[${req.pubId}] subscribe topic[${req.method}] params[${req.params}] successfully: ${subsId}`
        );
    } else {
        // rpc request, maybe big data package
        log.info(
            `New web socket response puber[${req.pubId}] method[${req.method}] params[${req.params}]`
        );
...
const recoverPuberTopics = (puber: Puber, ws: WebSocket, subsId: string) => {
    ...
    log.info(
        `Recover subscribed topic[${req.method}] params[${req.params}] of puber [${id}] done`
    );
};
...
const openHandler = async (
    ...
    log.info(
        `Into re-open handle chain[${chain}] suber[${subId}] pubers[${pubers}]`
    );
...
const newSuber = (
    ...
    ws.once("open", () => {
        ...
        if (isNone(re)) {
            log.error(
                `update suber on open error: invalid suber ${suber.id} chain ${chain} type ${type}`
            );
            process.exit(2);
        }
    ...
    ws.on("close", async (code: number, reason: string) => {
        ...
        if (isNone(re)) {
            log.error(
                `Handle suber close event error: invalid suber ${suber.id} of chain ${chain}`
            );
            process.exit(1);
        }
        ...
        delays(5, () => {
            log.warn(
                `create new suber try to connect ${
                    curTryCnt + 1
                } times, pubers `,
                pubers
            );
    ...
    ws.on("message", (dat: WebSocket.Data) => {
        ...
        if (re.value === true) {
            log.info(
                `unsubscribe topic done after puber closed: ${Util.globalStat()}`
            );
            return;
        }
        ...
        puberSend(req.pubId, data);
        log.info(
            `new suber message of [${req.method}] parse time[${time}]:  `,
            Util.globalStat()
        );
    });
    return suber;
...
namespace Suber {
    ...
    export const selectSuber = async (
        chain: string,
        type: SuberTyp
    ): PResultT => {
    ...
    export const initChainSuber = async (chain: string, poolSize: number) => {
        ...
        log.info(
            `Url of chain [${chain}] is: ${urls} kv enable [${conf.kvEnable}]`
        );
    ...
    export const init = async () => {
        ...
        log.info(
            `NODE_ENV is ${process.env.NODE_ENV}, pool size ${wsConf.poolSize}`
        );
    ...
    export const unsubscribe = async (
        ...
        if (isNone(re)) {
            log.error(
                `[SBH] get suber to unsubcribe error: invalid suber ${subId} of chain ${chain}`
            );
            process.exit(1);
        }
        ...
        log.info(
            `Suber[${subId}] send unsubcribe topic[${topic}] id[${subsId}] of chain ${chain} `,
            chain,
            subId,
            topic,
            subsId
