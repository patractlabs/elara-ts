...
const clearSubContext = async (puber: Puber, code: number) => {
    ...
    log.info(
        `clear subscribe context chain[${chain}] pid[${pid}] code[${code}] topics: `,
        Object.keys(topics)
    );
    ...
    if (!hasTopic) {
        ...
        for (let reqId in reqs) {
            ...
            if (req.pubId === puber.id) {
                G.delReqCache(reqId);
                log.warn(
                    `clear request cache [${reqId}] of puber[${puber.id}]`
                );
            }
        }
    ...
    for (let subsId of puber.topics || new Set()) {
        ...
        log.info(
            `unsubscribe topic[${subscript.method}] id[${subsId}] of chain ${chain} pid[${pid}] suber[${subId}]`
        );
    }
};
...
const connLimit = async (ws: WebSocket, chain: string, pid: IDT): PResultT => {
    ...
    log.info(
        `current ws connection of chain ${chain} pid[${pid}]: ${curConn}/${wsConf.maxConn}`
    );
...
namespace Matcher {
    export const regist = async (
        ...
        log.info(
            `regist puber[${puber.id}] to suber[${suber.id}]: `,
            Util.globalStat()
        );
        return Ok(puber);
    };
    ...
    export const newRequest = (
        ...
        if (isUnsubReq(method)) {
            log.info(
                `pre handle unsubscribe request: ${method}: `,
                data.params,
                Suber.isSubscribeID(data.params[0])
            );
            type = ReqTyp.Unsub;
            if (
        ...
        data.id = req.id;
        log.info(
            `global stat after new request[${req.id}] : `,
            Util.globalStat()
        );
        return Ok(data);
    };
    ...
    export const setSubContext = (req: ReqT, subsId: string): ResultT => {
        ...
        G.addSubTopic(puber.chain, puber.pid, {
            id: subsId,
            pubId: req.pubId,
            method: req.method,
            params: req.params,
        ...
        log.info(
            `After set subscribe context requestId[${req.id}] global stat: `,
            Util.globalStat()
        ); // for test
        return Ok(0);
    };
    ...
    export const unRegist = async (pubId: IDT, code: number): PVoidT => {
        ...
        if (Result.isErr(re) || !re.value.subId) {
            // SBH
        ...
        log.info(
            `Unregist successfully: ${pubId} - ${suber.id}, global stat: `,
            Util.globalStat()
        );
        // global.gc()
    };
    ...
    export const getSuber = (chain: string, pubId: IDT): ResultT => {
        ...
        if (Result.isErr(re)) {
            return Err(
                `No valid suber of chain[${chain}]-subID[${puber.subId}]`
            );
        }
    ...
    export const isSubscribed = (
        ...
        const topics = G.getSubTopics(chain, pid);
        log.info(
            `subscribed topics of chain[${chain}] pid[${pid}]: `,
            Object.keys(topics)
        );
