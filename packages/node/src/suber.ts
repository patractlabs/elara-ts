...
const closeHandler = async (chain: string, suber: Suber): PVoidT => {
    log.warn(
        `Try to connect chain[${chain}] failed out of limit, start to clear suber resource.`
    );
    // clear pubers context
    if (!suber.pubers) {
        log.info(
            `No pubers need to be clear  chain ${chain} suber[${suber.id}]`
        );
        return;
    }
...
const openHandler = async (
    ...
    log.info(
        `Into re-open handle chain[${chain}] suber[${subId}] pubers[${pubers.values()}]`
    );
...
namespace Suber {
    ...
    export const initChainSuber = async (chain: string, poolSize: number) => {
        ...
        if (serConf.id != (conf.value as ChainConfig).serverId) {
            log.warn(
                `Chain[${chain}] bind to server ${conf.value.serverId}, skip! current server is ${serConf.id}`
            );
            return;
        }
        G.addChain(chain);
