...
namespace Puber {
    ...
    export const updateTopics = (dispId: IDT, subsId: string): ResultT => {
        ...
        if (Result.isNone(re)) {
            log.error(
                `update puber[${dispId}] topics error: no this puber ${dispId}`
            );
            // puber may be closed
        ...
        log.info(
            `update puber[${dispId}] topic [${subsId}] done: ${puber.topics.values()}`
        );
        return Ok(puber);
    };

    export const transpond = async (
        puber: Puber,
        type: SuberTyp,
        data: ReqDataT
    ): PVoidT => {
        ...
        if (Matcher.isSubscribed(chain, pid, data)) {
            log.warn(
                `The topic [${data.method}] has been subscribed, no need to subscribe twice!`
            );
        ...
        if (Result.isNone(re)) {
            log.error(
                `[SBH] send message error: invalid suber ${puber.subId} chain ${chain}`
            );
            process.exit(1);
        }
        ...
        // transpond requset
        log.info(
            `Send new message to suber[${suber.id}] of chain ${chain}, request ID: ${dat.id}`
        );
